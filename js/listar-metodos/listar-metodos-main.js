...
const aplicarModo = (modo = 0) => {
    ...
    if (modo == 1) {
        const mainTitle = document.querySelector("body > section > div > div.header > h2");

        mainTitle.textContent = 'Seleccionar Método de Pago';

        const addTitle = document.querySelector("body > section > div > div.caja-botones > p");

        addTitle.textContent = '';

        const addImageBox = document.querySelector("body > section > div > div.caja-botones > div")
    ...
    if (typeof tarjetas !== 'undefined') {
        ...
        cajasTarjeta.forEach(card => {
            ...
            if (modo !== 1) {
                ...
                const deleteOptions = document.querySelectorAll('.contenedor-tarjeta .tarjeta .links-box .eliminar');

                deleteOptions.forEach((option) => {
                    option.addEventListener('click', removeCard);
                });


                const modifyOptions = document.querySelectorAll('.contenedor-tarjeta .tarjeta .links-box .modificar');

                modifyOptions.forEach((option) => {
                    option.addEventListener('click', modifyCard);
                });
...
const initAPP = async() => {
    ...
    try {
        ...
        const tarjetasInDB = await obtenerTarjetasUsuario()
            .then((tarjetasDB) => {
                // tarjetas.length = 0;
                tarjetas = tarjetasDB.map((tarjeta) => ({ id: tarjeta._id, number: Number(`${tarjeta.number.toString().slice(0,4)}`) }));
